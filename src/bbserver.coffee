            ...
            try
                ...
                if config.get('bubblebot_use_https')
                    ...
                    if not cert
                        ...
                        setTimeout ->
                            u.report 'Config option bubblebot_use_https is turned on, but we were not able to load a key.  Use the set_config command with key "ssl_key" to set a key'
                        , 20000
...
class New extends Command
    ...
    params: [
        ...
        {name: 'template', type: 'list', options: (-> templates.list('Environment')), required: true, help: "The environment template to use to build this environment.  Pass 'blank' to create an empty environment"}
